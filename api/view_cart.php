<?php
// Ensure session starts before reading session array
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

// 1. Include DB Connection cleanly relative to this file
...
// 3. Load Header cleanly relative to this directory
require_once __DIR__ . "/store_header.php"; 

// 4. Initialize multidimensional cart array structure
...
$pharmacy_name = $pharmacy_name ?? 'Pharmacy';
?>
...
<head>
    ...
    <style>
        ...
        .btn-checkout:hover { 
            ...
            box-shadow: 0 5px 15px rgba(0,179,134,0.4); 
            color: white;
        }
        .empty-cart-state { 
            padding: 80px 20px; 
                        ...
                        <table class="table cart-table mb-0">
                            ...
                            <tbody>
                                <?php foreach($active_cart as $id => $item): 
                                    ...
                                    $sub        = $item_price * $item_qty;
                                    $grand_total += $sub;
                                ?>
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="product-name"><?php echo htmlspecialchars($item['name'] ?? 'Unknown Item'); ?></div>
                                        </div>
                                    </td>
                                    <td class="fw-bold">K <?php echo number_format($item_price, 2); ?></td>
                                    <td>
                                        <div class="qty-group">
                                            <button type="button" class="qty-btn" onclick="changeQty(this, -1)">-</button>
                                            <input type="text" name="qty[<?php echo $id; ?>]" class="qty-input" value="<?php echo $item_qty; ?>" readonly>
                                            <button type="button" class="qty-btn" onclick="changeQty(this, 1)">+</button>
                                        </div>
                                    </td>
