...
// Resolve requested branch, but only if it exists and is active.
$requested_branch = isset($_GET['bid']) ? (int)$_GET['bid'] : 0;
if ($requested_branch > 0) {
    $stmt = $conn->prepare("SELECT id FROM branches WHERE id = ? AND is_active = 1 LIMIT 1");
    if ($stmt) {
        $stmt->bind_param('i', $requested_branch);
        $stmt->execute();
        $valid = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        if ($valid) $_SESSION['current_branch_id'] = $requested_branch;
    }
}
...
<div class="dropdown">
<div class="location-selector dropdown-toggle" data-bs-toggle="dropdown" role="button"><i class="mdi mdi-map-marker-outline fs-6"></i> <span class="text-primary"><?php echo htmlspecialchars($branch_name); ?></span></div>
...
<li><a class="dropdown-item <?php echo ((int)$bl['id']===$branch_id)?'active':''; ?>" href="online_store.php?bid=<?php echo (int)$bl['id']; ?>"><?php echo htmlspecialchars($bl['branch_name']); ?></a></li>
...
<a href="online_store.php?bid=<?php echo $branch_id;?>" class="strip-link <?php echo $category_filter===''?'active':'';?>">All Products</a>
<?php if($cat_query):while($c=$cat_query->fetch_assoc()):$cat_name=$c['name']??'';?><a href="online_store.php?bid=<?php echo $branch_id;?>&cat=<?php echo urlencode($cat_name);?>" class="strip-link <?php echo strcasecmp($category_filter,$cat_name)===0?'active':'';?>"><?php echo htmlspecialchars($cat_name);?></a><?php endwhile;endif;?>
</div></div>
...
<div class="col-12 col-md-5"><form action="online_store.php" method="GET" class="hng-search-container"><input type="hidden" name="bid" value="<?php echo $branch_id;?>"><select name="type"><option value="all" <?php echo (($_GET['type']??'all')!=='rx')?'selected':'';?>>All</option><option value="rx" <?php echo (($_GET['type']??'')==='rx')?'selected':'';?>>Rx</option></select><input type="search" name="q" value="<?php echo htmlspecialchars($_GET['q']??'');?>" placeholder="Search medicines in <?php echo htmlspecialchars($branch_name);?>..." autocomplete="off"><button type="submit" class="hng-search-btn" aria-label="Search"><i class="mdi mdi-magnify"></i></button></form></div>
<div class="col-12 col-md-4 mt-2 mt-md-0"><div class="nav-icon-grid"><a href="online_store.php?bid=<?php echo $branch_id;?>" class="hng-nav-icon"><i class="mdi mdi-home"></i>Home</a><a href="categories.php?bid=<?php echo $branch_id;?>" class="hng-nav-icon"><i class="mdi mdi-view-grid"></i>Category</a><a href="offers.php?bid=<?php echo $branch_id;?>" class="hng-nav-icon"><i class="mdi mdi-label-percent"></i>Offer</a><a href="help.php?bid=<?php echo $branch_id;?>" class="hng-nav-icon"><i class="mdi mdi-help-circle"></i>Help</a><a href="javascript:void(0)" onclick="openBankDetails()" class="hng-nav-icon"><i class="mdi mdi-bank"></i>Bank</a></div></div>
</div></div></div>
