...
if ($category_filter !== '') {
    $product_sql .= " AND category = ?";
    $types .= 's';
    $params[] = $category_filter;
}

if ($search_query !== '') {
    $product_sql .= " AND (item_name LIKE ? OR strength LIKE ? OR category LIKE ?)";
    $like = '%' . $search_query . '%';
    $types .= 'sss';
    $params[] = $like;
    $params[] = $like;
...
$product_sql .= " ORDER BY item_name ASC LIMIT 60";
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 mb-md-4">
        <div>
            <h4 class="fw-bold mb-0 fs-5 fs-md-4">
                <?php echo $search_query !== '' ? 'Search Results' : 'Shop Medicines'; ?>
            </h4>
            <?php if ($search_query !== ''): ?>
                <small class="text-muted">Results for â€œ<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>â€</small>
            <?php endif; ?>
        </div>
