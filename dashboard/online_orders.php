...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3"><div><h4 class="oo-title mb-1">Online Orders</h4><div class="text-muted small">Customer orders received from the online pharmacy store.</div></div><button class="oo-btn oo-primary" onclick="location.reload()">↻ Refresh</button></div>
...
<div class="oo-modal" id="ooModal"><div class="oo-dialog"><div class="p-3 border-bottom d-flex justify-content-between"><strong id="ooTitle">Order</strong><button class="btn btn-light rounded-circle" onclick="closeOrder()">×</button></div><div class="p-3" id="ooBody">Loading…</div></div></div>
<script>
function esc(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));}
async function viewOrder(id){const r=await fetch('online_orders.php?action=order&id='+id);const d=await r.json();if(!d.success){alert(d.message||'Unable to load order.');return}const o=d.order;let h='<div class="mb-3"><b>Customer:</b> '+esc(o.full_name||'Customer')+'<br><b>Phone:</b> '+esc(o.phone||'')+'<br><b>Branch:</b> '+esc(o.branch_name||'')+'<br><b>Payment:</b> '+esc(o.payment_method||'')+'<br><b>Status:</b> '+esc(o.status)+'</div>';h+=d.items.map(i=>'<div class="oo-item"><span><b>'+esc(i.item_name||'Product')+'</b><br><small class="text-muted">'+esc(i.strength||'')+' × '+i.quantity+'</small></span><strong>K'+Number(i.price_at_purchase).toFixed(2)+'</strong></div>').join('');h+='<div class="text-end fs-5 mt-3"><b>Total: K'+Number(o.total_amount).toFixed(2)+'</b></div>';document.getElementById('ooTitle').textContent='#'+o.order_number;document.getElementById('ooBody').innerHTML=h;document.getElementById('ooModal').classList.add('open');}
