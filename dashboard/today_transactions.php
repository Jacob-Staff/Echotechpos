...
// 1. Fetch Branding
$info_res = mysqli_query($conn, "SELECT p.name, b.branch_name FROM pharmacies p JOIN branches b ON b.pharmacy_id = p.id WHERE p.id = $p_id AND b.id = $b_id LIMIT 1");
$info = mysqli_fetch_assoc($info_res);
$display_pharm = $info['name'] ?? 'PHARMANOVA';
$display_bran  = $info['branch_name'] ?? 'Pharmanova LSK';

// 2. UPDATED QUERY: Filter by $filter_date instead of hardcoded today
$sql = "SELECT s.*, u.username as issuer,
        ...
        (SELECT SUM(total_amount) FROM sales WHERE pharmacy_id = $p_id AND branch_id = $b_id AND DATE(created_at) = '$filter_date') as day_total,
        (SELECT COUNT(*) FROM sales WHERE pharmacy_id = $p_id AND branch_id = $b_id AND DATE(created_at) = '$filter_date') as day_count
        FROM sales s
        LEFT JOIN users u ON s.user_id = u.id
        WHERE s.pharmacy_id = $p_id 
        AND s.branch_id = $b_id 
