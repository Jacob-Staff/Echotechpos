...
$invoiceSql = "
    SELECT
        ...
        z.id AS zra_record_id,
        z.status AS zra_status,
        z.environment AS zra_environment,
        z.zra_invoice_no,
        z.zra_sdc_id,
        z.fiscal_signature,
        z.internal_data,
    ...
    LEFT JOIN pos_zra_invoices z
        ON z.sale_id = s.id
       AND z.pharmacy_id = s.pharmacy_id
       AND z.branch_id = s.branch_id
    ...
    <div class="page-wrapper invoice-page">
        ...
        <div class="container-fluid">
            ...
            <div class="invoice-card">
                ...
                <!-- SMART INVOICE / ZRA FISCAL STATUS -->

                <?php if (!empty($invoice['zra_record_id'])): ?>
                <div class="reference-box" style="margin-top:24px;padding:15px;border:1px solid #e2e8f0;border-radius:10px;background:#f8fafc;">
                    <div class="reference-label">Smart Invoice / ZRA</div>
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:8px;">
                        <div><span class="meta-label">Status</span><span class="meta-value"><?= invoice_e(ucwords(str_replace('_',' ',(string)$invoice['zra_status']))) ?></span></div>
                        <div><span class="meta-label">ZRA Invoice</span><span class="meta-value"><?= invoice_e($invoice['zra_invoice_no'] ?: 'Pending') ?></span></div>
                        <div><span class="meta-label">SDC ID</span><span class="meta-value"><?= invoice_e($invoice['zra_sdc_id'] ?: 'Pending') ?></span></div>
                    </div>
                    <?php if (!empty($invoice['fiscal_signature'])): ?>
                    <div class="reference-box" style="margin-top:12px;">
                        <div class="reference-label">Fiscal Signature</div>
                        <div class="reference-value"><?= invoice_e($invoice['fiscal_signature']) ?></div>
                    </div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
