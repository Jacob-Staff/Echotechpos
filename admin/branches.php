                    ...
                    <?php else: ?>
                        <?php foreach ($branches as $b): ?>
                            ...
                            <tr>
                                ...
                                <td class="right">
                                    <div class="row-actions" aria-label="Branch actions">
                                        <a class="action-btn view" href="view_branch.php?id=<?= (int)$b['id'] ?>" title="Open branch dashboard"><i class="fas fa-chart-line"></i><span>View</span></a>
                                        <button class="action-btn edit" type="button" title="Edit branch" onclick='editBranch(<?= json_encode($payload, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_HEX_QUOT) ?>)'><i class="fas fa-pen"></i><span>Edit</span></button>
                                        <form method="post" class="inline-form" onsubmit="return confirm(<?= json_encode(((int)$b['is_active'] === 1) ? 'Deactivate this branch? Existing records will remain safe and the branch can be reactivated later.' : 'Activate this branch?') ?>);">
                                            ...
                                            <button class="action-btn <?= (int)$b['is_active'] === 1 ? 'warning' : 'success' ?>" type="submit" title="<?= (int)$b['is_active'] === 1 ? 'Deactivate branch' : 'Activate branch' ?>">
                                                <i class="fas <?= (int)$b['is_active'] === 1 ? 'fa-pause' : 'fa-play' ?>"></i><span><?= (int)$b['is_active'] === 1 ? 'Deactivate' : 'Activate' ?></span>
                                            </button>
                                        </form>
                                        <form method="post" class="inline-form" onsubmit="return confirm('Permanently delete this branch? This is only allowed when the branch has no linked operational records.');">
                                            ...
                                            <input type="hidden" name="id" value="<?= (int)$b['id'] ?>">
                                            <button class="action-btn danger" type="submit" title="Delete branch"><i class="fas fa-trash"></i><span>Delete</span></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
...
.row-actions{display:flex;justify-content:flex-end;align-items:center;gap:6px;flex-wrap:wrap;min-width:430px}
.action-btn{height:34px;min-width:74px;padding:0 10px;border:1px solid var(--border);background:#fff;color:#4f5c68;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;gap:6px;cursor:pointer;text-decoration:none;font:inherit;font-size:9px;font-weight:850;white-space:nowrap}
.action-btn i{font-size:9px}
.action-btn:hover{border-color:#a9c0ec;color:var(--blue);background:#fbfdff}
.action-btn.warning{color:#9a6d11;background:var(--yellow-soft);border-color:#f2dfae}
.action-btn.success{color:#19764f;background:var(--green-soft);border-color:#c6e8d8}
.action-btn.danger{color:#c54a5c;background:var(--red-soft);border-color:#f0c5cc}
