...
// Staff Management is operational for Admin and Human Resource.
// Admin retains full security/permissions control; HR manages employee accounts.
$user_role = current_role();
if (!in_array((string)$user_role, ['Admin', 'Human Resource'], true)) {
    http_response_code(403);
    exit('Access denied. Staff Management is restricted to Admin and Human Resource.');
}
$is_admin = ($user_role === 'Admin');
$is_hr = ($user_role === 'Human Resource');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    $allowed_roles = ['Human Resource', 'Pharmacist', 'Cashier', 'Manager', 'User'];
    if (!in_array($role, $allowed_roles, true)) {
        $role = 'User';
    }
    $branch_id = intval($_POST['branch_id']);
    $salary = max(0, floatval($_POST['salary'] ?? 0));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_permission'])) {
    if (!$is_admin) {
        http_response_code(403);
        exit('Only Admin can change role permissions.');
    }
...
<head>
    <meta charset="utf-8">
    <title>Staff & Permissions | EchoTech POS</title>
            ...
            <table class="table table-dark table-bordered text-center align-middle mb-0">
                <thead class="table-secondary text-dark">
                    <tr>
                        ...
                        <th>Human Resource</th><th>Pharmacist</th><th>Manager</th><th>Cashier</th><th>User</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $modules = ['Employee Management', 'Inventory', 'Sales', 'Suppliers', 'Expenses', 'Reports'];
                    $roles = ['Human Resource', 'Pharmacist', 'Manager', 'Cashier', 'User'];
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="small text-info mt-2">
                                    ...
                                    Human Resource accounts automatically receive access to the Employee Management portal.
                                </div>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="small text-muted">Initial Salary (optional)</label>
                                <input type="number" step="0.01" name="salary" class="form-control bg-secondary text-white border-0" value="0" readonly>
                            </div>
                        </div>
                        <div class="mb-3">
