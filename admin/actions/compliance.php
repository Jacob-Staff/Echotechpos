...
<?php elseif ($view === 'invoices'): ?>
    ...
    <?php if(!$zraInvoices): ?><tr><td colspan="6" class="empty">No sales records were found for this pharmacy/branch.</td></tr><?php else: foreach($zraInvoices as $s): ?><tr><td>#<?= (int)$s['id'] ?></td><td><?= compliance_h($s['invoice'] ?? '—') ?></td><td><?= !empty($s['created_at'])?compliance_h(date('d M Y H:i',strtotime($s['created_at']))):'—' ?></td><td><?= compliance_money((float)($s['total'] ?? 0)) ?></td><td><?= compliance_h($s['payment_method'] ?? '—') ?></td><td><span class="badge amber">Phase 1 only</span></td></tr><?php endforeach; endif; ?></tbody></table></div></section>
